...
app.get('/', async function(req, res) {
    try {
      ...
      if (req.session.username) {
        res.render('/home', { username: req.session.username, songs: songs });
      } else {
...
app.post('/removeFromPlaylist', async (req, res) => {
    try {
        ...
        const index = user.workingPlaylist.indexOf(songId);
        if (index !== -1) {
            user.workingPlaylist.splice(index, 1);
            await user.save();
